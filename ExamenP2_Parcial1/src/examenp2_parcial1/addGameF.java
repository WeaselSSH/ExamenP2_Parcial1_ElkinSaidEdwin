...
public class addGameF extends FrontEnd {
    ...
    private final JDateChooser dcPublicacion = new JDateChooser();
    ...
    public addGameF(SistemaWonderland sistema) {
        ...
        JLabel titulo = new JLabel("Agregar Juego");
        titulo1(titulo);
    ...
    private JPanel crearPanelFormulario() {
        ...
        gbc.gridy = 2; agregarCampo(panel, gbc, "Copias Disponibles:", campoCopias);
        
        gbc.gridy = 3; agregarCampo(panel, gbc, "Buscar Imagen:", filaSoloBotonImagen(btnBuscarImagen));
        gbc.gridy = 4; configurarDateChooser(dcPublicacion); agregarCampo(panel, gbc, "Fecha de publicacion:", dcPublicacion);
    ...
    private void seleccionarImagen() {
        ...
        ch.setFileFilter(new FileNameExtensionFilter("Imágenes", "jpg", "jpeg", "png", "gif"));
    ...
    private void guardarJuego() {
        try {
            ...
            if (nombre.isEmpty() || rutaImagenSeleccionada == null || rutaImagenSeleccionada.trim().isEmpty()) {
                JOptionPane.showMessageDialog(this, "Todos los campos son obligatorios, incluyendo la imagen.", "Validacion", JOptionPane.WARNING_MESSAGE);
                return;
            }
            if (copias < 0) {
                JOptionPane.showMessageDialog(this, "El numero de copias no puede ser negativo.", "Validacion", JOptionPane.WARNING_MESSAGE);
                return;
            }
            ...
            if (dcPublicacion.getDate() == null) {
                JOptionPane.showMessageDialog(this, "Debe seleccionar una fecha de publicacion.", "Validacion", JOptionPane.WARNING_MESSAGE);
                return;
            }

            Game nuevoJuego = new Game(codigo, nombre, rutaImagenSeleccionada);
            nuevoJuego.setCantidadCopias(copias);
            Calendar cal = Calendar.getInstance();
            cal.setTime(dcPublicacion.getDate());
            nuevoJuego.setfechaPublicacion(cal.get(Calendar.YEAR), cal.get(Calendar.MONTH) + 1, cal.get(Calendar.DAY_OF_MONTH));
            sistemaWonderland.agregarItem(nuevoJuego);

            int resp = JOptionPane.showConfirmDialog(this, "Juego agregado correctamente. ¿Desea agregar otro?", "Exito", JOptionPane.YES_NO_OPTION);
            ...
            JOptionPane.showMessageDialog(this, "Verifica formatos: codigo y copias deben ser numeros enteros.", "Error de formato", JOptionPane.ERROR_MESSAGE);
        } catch (Exception ex) {
            JOptionPane.showMessageDialog(this, "No se pudo agregar el juego: " + ex.getMessage(), "Error", JOptionPane.ERROR_MESSAGE);
        }
    }
    ...
    private void limpiarCampos() {
        ...
        preview.setIcon(null);
        dcPublicacion.setDate(new Date());
       
        estilizarDateChooser(dcPublicacion);
